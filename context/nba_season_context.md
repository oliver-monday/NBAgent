...
**DET — AST suppressor (WATCH — below formal gate, extreme magnitude):** System AST hit rate vs. Detroit is 50.0% (n=8), −37.2pp below the AST baseline. Below the ≥15 pick formal confirmation threshold but the magnitude is the largest in the dataset. DET's switching/interior defense compresses secondary-creator AST production. Do not hard-skip AST picks vs. DET, but apply strong scrutiny to non-primary-creator AST props. Primary playmakers (raw_avgs AST ≥ 6.0) may be less affected — the signal is concentrated on secondary creators (Miller 0 AST, Knueppel 1 AST on 4/10). **PLAYOFF RELEVANCE: DET is the 1-seed East. Every Eastern Conference playoff team may face DET. Monitor AST picks vs. DET carefully — if the signal holds through R1 (4 more AST picks would cross the ≥15 gate), upgrade to confirmed suppressor.**
