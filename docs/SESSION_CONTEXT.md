...
**Last updated:** April 9, 2026 ((39) **Pre-Tip Odds Sweep (Layer 1)** — `ingest/odds_today.py`: new `--pretip` CLI mode runs `pretip_sweep()` — fetches FanDuel odds for games tipping within configurable window (default 75 min), updates `picks.json`, recomputes calibrated edge, logs line movement vs morning baseline. `_parse_fanduel_outcomes()` extracted from `main()` inline parsing — shared by `main()` and `pretip_sweep()`. `main()` now saves morning baseline to `data/odds_pretip.json` via `_save_morning_baseline()`. Pre-tip sweeps append per-event snapshots (deduped by event_id). New workflow `.github/workflows/odds_pretip.yml`: 10 cron entries every 30 min 3:00–7:30 PM PT. Most runs no-op (0 credits). Also this session: `walked_tier` field + reconciler rewrite, `enforce_market_gate()` Python post-processor, alt-tier edge display in Odds+Sizing drawer, trim badge removal, pre-game reporter staleness token fix, calibration key mismatch fix, post-processor greedy regex fix, frontend odds display fixes, Best Bets section, parlay JSON extraction fallback, auditor.yml bs4/rotowire fixes, injuries.yml last-run trim. `ingest/odds_today.py`, `.github/workflows/odds_pretip.yml` (NEW).)
