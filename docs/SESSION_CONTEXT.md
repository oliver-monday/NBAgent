...
- `save_scout_omitted(omitted: list[dict]) → None` — writes `data/scout_omitted_YYYY-MM-DD.json` with Scout's omitted block; called in `main()` immediately before Stage 2 Pick call; added 2026-03-20. **Omitted schema (updated 2026-03-23):** two categories — (1) hard exclusions (players not shortlisted at all: no qualifying tier, confirmed OUT, role eliminated) and (2) deprioritized candidates (shortlisted at `priority: "medium"` with meaningful situational risk — B2B, VOLATILE sole prop, thin minutes floor, QUES designation, extreme blowout as secondary scorer); `risk_type` field added (`hard_exclusion` | `deprioritized` | `none`); omitted array is now required — never empty; sentinel entry `{"player_name": "none", ..., "risk_type": "none"}` used when all candidates are clean. Previously the array was always `[]` on typical 12–15 player slates because the Scout shortlisted everyone.
...
- Skip reasons: `min_floor_tier_step`, `volatile_weak_combo`, `blowout_secondary_scorer`, `3pm_trend_down_tough_dvp`, `3pm_trend_down_low_minutes`, `3pm_blowout_trend_down` (also reused for extreme blowout at spread_abs ≥ 19 regardless of trend direction — 2026-03-20), `ast_hard_gate`, `fg_margin_thin_tier_step`, `reb_floor_skip`, `fg_cold_tier_step`, `volatile_ast_t6`, `blowout_t25_skip`
