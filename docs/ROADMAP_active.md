...
**P2 (Kalshi frontend display)** — RESOLVED 2026-05-01. See `ROADMAP_resolved.md`. Side-by-side FanDuel + Kalshi book quote lines (American odds + implied prob) now render in the right rail of all three pick-card surfaces in `agents/build_site.py`. No computed delta — visual divergence read at a glance.

**W10 — P2 frontend rendering verification** — Confirm FanDuel and Kalshi lines render on all three card surfaces (Today's Picks, Top Picks, Best Bets) on next site rebuild. Coverage matrix: both books listed → real values both lines; FanDuel listed + Kalshi `false`/absent → "Kalshi: —"; FanDuel null (defensive only) → "FanDuel: —". Verify monospace alignment makes American-odds divergence visible at a glance.

**P4 (Kalshi-only toggle filter)** — RESOLVED 2026-05-01. See `ROADMAP_resolved.md`. "Kalshi-listed only" toggle added below the HC toggle on Today's Picks tab. Filter composes with existing HC toggle via logical AND. The original P4 scope included an auditor coverage warning at <50% Kalshi coverage; that sub-item is intentionally dropped — Kalshi has structurally fewer tiers than FanDuel, so low coverage days are expected and not a system health signal.

**W11 — P4 Kalshi-only toggle verification** — Confirm filter composes with HC toggle (AND logic), counters update correctly, page reload resets both toggles to OFF, no regression in pre-P4 HC filter behavior. Also confirm `data-kalshi` attribute writes 'true'/'false' correctly on all three card surfaces (Today's Picks, Top Picks, Best Bets).

**W12 — MARKET_GATE_REJECT structured logging verification** — Confirm next analyst run with at least one gate violation produces parallel records in `data/skipped_picks.json` (`skip_reason="market_gate_reject"`) matching the `[analyst] MARKET_GATE_REJECT:` console lines 1:1. If no violations occur for several days (the common case post-2026-04-26 prompt rule fix), the field is silently invisible — that's healthy, not a problem. Validate `rule_context.failure_mode` is one of `{player_absent, tier_absent}` and `rule_context.available_tiers` is `null` for player_absent or a sorted integer list for tier_absent.
...
- **Miss classification analysis** — see `data/miss_classification_report.md` (generated by `tools/miss_classification.py`) and `ROADMAP_resolved.md` 2026-05-01 entry. The report's Findings section names candidate workstream areas (largest catchable bucket: `deterministic_rule_catchable` 69% of catchable population; top pattern: `blowout`; calibration check flags 85% razor/small variance margins suggesting recalibration is the dominant lever) but does not prescribe rule changes. Next-workstream decision is an offseason planning conversation; if a specific workstream is chosen, add a new open item at that point.
