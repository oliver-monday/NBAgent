...
- `ROTOWIRE_MINUTES_URL = "https://www.rotowire.com/basketball/projected-minutes.php"` — constant added after `ROTOWIRE_LOGIN_URL`; diagnostic confirmed this page is fully statically rendered and contains all target CSS classes (`lineups-viz`, `minutes-meter__proj`, `lineups-viz__player-name`)
...
- `reconcile_pick_values(picks: list[dict]) → list[dict]` — post-processes picks before writing to disk; detects mandatory tier step-downs documented in `tier_walk` but not reflected in `pick_value` (root cause: Claude reasons correctly about step-downs but Python never verified the two fields agreed); two-strategy parser: (1) rightmost valid tier after a step/→/apply/drop keyword that is lower than current `pick_value`; (2) fallback: rightmost `→T{N}` arrow pattern; never raises `pick_value`; on correction: updates `pick_value`, appends `[reconciled: pick_value corrected {old}→{new} by Python post-processor]` to `tier_walk`, prints `[analyst] RECONCILED:` warning; on ambiguous parse: prints `[analyst] RECONCILE_SKIP:` and leaves pick untouched; called as first line of `save_picks()`. Motivating failure: Jalen Duren PTS T15 March 12 — `tier_walk` documented "Apply step to T10" but `pick_value=15` was published; Duren scored 14 (HIT at T10, graded MISS at T15).
...
### `auditor.py`
- **Path constants:** `MASTER_CSV = DATA / "nba_master.csv"` added (alongside existing `PICKS_JSON`, `PARLAYS_JSON`, etc.)
...
**Also completed March 12 (Session 2):**
- **Analyst-driven Top Picks** — `top_pick: true/false` field added to analyst JSON schema; `## TOP PICKS — FINAL SELECTION STEP` section added to `build_prompt()` (after ANALYSIS APPROACH, before OUTPUT FORMAT); criteria: conf ≥78%, 8/10 hit rate, not VOLATILE, not BLOWOUT_RISK/B2B/DENSE, 2–4 picks per day; picks rule added requiring `top_pick=true` on exactly the flagged picks. `get_top_picks()` in `build_site.py` updated: primary gate is `p.get("top_pick") is True`; falls back to `confidence_pct >= 85` for backwards-compat (old picks). `past_top_picks` filter updated to `top_pick=True OR conf>=85`. `skip_reason` enum unchanged.
- **Results tab redesign** — `<div id="tab-results">` now has a 3-column card row (Overall | Yesterday | ⚡ Top Picks) + full-width Props card (4 streak cards) + chart + three history drawers. `overall-banner` removed; new CSS classes: `.results-cards-row`, `.results-card`, `.results-card-wide`, `.results-hero-num`, `.results-hero-sub`. Yesterday card populated from `DATA.last_audit` in `renderResults()`. Top Picks card populated from `DATA.top_picks_history` (requires ≥3 graded). Props card contains existing `prop-streak-grid` unchanged.
- **`load_yesterday_summary()`** — new function in `build_site.py` reads `AUDIT_SUMMARY_JSON` (`data/audit_summary.json`); returns `{}` gracefully if missing; wired into `build_site()` as `yesterday_summary`; serialised as `yesterday_summary_json` in `generate_html()` and added to JS `DATA` block. Currently available in JS but not yet rendered (reserved for future cards if needed).
- **Post-game reporter bug fixes** — `load_yesterdays_missed_pick_names()` changed to `result == "MISS"` only (was `result in ("MISS", None)` — caused all 14 players to be treated as missed before auditor graded them); `espn_fetch_ok` scoping fix (variable now initialised to `None` before `if aid:` block, so field is always present in `players_out`; renamed to `"espn_fetch_ok"` in output dict).

**Also completed March 12 (Session 1):**
...
**Also completed March 13, 2026:**
- **`agents/analyst.py` — tier step-down pick_value reconciliation bug fix** — `VALID_TIERS` constant added after `LARGE_SLATE_THRESHOLD`; `reconcile_pick_values()` function added and called as the first line of `save_picks()`; catches cases where Claude correctly documented a mandatory tier step-down in `tier_walk` but `pick_value` was not updated to match; only lowers pick_value, never raises; skips ambiguous cases with a warning log. Motivating failure: Jalen Duren PTS T15 March 12 — `tier_walk` said "Apply step to T10", `pick_value` published as 15, actual score 14, graded MISS at T15 instead of HIT at T10. Auditor classified `model_gap_rule`.
- **`agents/auditor.py` — voided pick exclusion from audit counts** — `build_audit_prompt()` now splits `graded_picks` into `voided_picks` / `active_picks` at entry; all counting and statistical breakdowns use `active_picks`; OUTPUT FORMAT emits `voided_picks` field alongside `total_picks`; `save_audit_summary()` now accumulates `total_voided` from `voided_picks` entries across audit log and writes `overall.voided` to `audit_summary.json`.
- **`agents/build_site.py` — voided count display + implied odds removal** — Results tab yesterday banner now appends `· N voided` when `voided_picks > 0`; Audit Log tab stats banner now shows a Voids figure (reads `a.voided_picks||0`) alongside Total/Hits/Misses; `implied_odds` removed from two display locations (parlay history date span, main parlay card `<div class="parlay-odds">`); `.parlay-odds` CSS rule removed.
- **`agents/build_site.py` — Research tab (Player Explorer)** — fifth tab added to the static SPA. `build_explorer_data()` reads `player_game_log.csv` + `nba_master.csv`; filters to active whitelist players; excludes DNPs; joins on `game_id` (`_gid` normalization: `astype(str).str.split(".").str[0].str.strip()` — same pattern as quant.py) for spread and game result context; stores `player_spread` (signed: negative=favorite, positive=underdog) + `spread_abs`; computes rest_bucket (0=B2B, 1, 2, 3+); prints match rate diagnostic; embeds per-player game arrays as `DATA.explorer` JSON in the HTML. `renderResearch()` builds 8-dropdown filter UI (player, stat, H/A, rest, fav/dog, spread bucket, result, opponent). `runExplorer()` applies all filters including favdog (`player_spread < 0` = favorite, `player_spread > 0` = underdog), renders tier hit rate table with bar charts, distribution stats, and full game log table (spread column shows signed `player_spread`) sorted newest-first. Fully static — no LLM calls, works on GitHub Pages.

**Also completed March 14, 2026:**
- **`agents/auditor.py` — voided pick grading bug fix** — `grade_picks()` had no guard for `voided=True` picks. Bug A: picks with `voided=True` received `result="MISS"` when player had 0 stats in box score (DNP). Bug B: picks with `void_reason` set but `voided=False` (post-hoc late-DNP promotion not completing) were graded `result="MISS", actual_value=0.0`. Fix 1A: guard at top of main loop body — `if p.get("voided") is True: set result=None/actual_value=None, continue`. Fix 1B: second-pass loop after main loop — detects `result=="MISS" AND actual_value==0.0 AND void_reason AND not voided` → promotes to `voided=True, result=None` with `[auditor] LATE_DNP_PROMOTED:` log. `build_audit_prompt()` and `save_audit_summary()` were already correct (correct voided/active split; denominator already `hits/(hits+misses)`). Historical repair: `scripts/repair_void_grading.py` patched 4 Derrick White picks (2026-03-12, Bug A) + 2 Alperen Sengun picks (2026-03-13, Bug B); corrected audit_log entries; regenerated audit_summary. Corrected season: 299H/51M = 85.4% (was 84.0% with 6 corrupted misses).
- **`agents/build_site.py` — Player Explorer join fix + signed spread + Fav/Dog filter** — `build_explorer_data()` join rewritten from `(game_date_str, team_abbrev)` to `game_id` (_gid normalization) matching quant.py pattern; achieves 87%+ match rate vs near-zero before. `player_spread` field added (signed: negative=favorite, positive=underdog; derived from `home_spread`/`away_spread` in nba_master.csv + home/away determination from master columns). Fav/Dog dropdown added to `renderResearch()` filter grid (before Spread dropdown); `runExplorer()` reads `ex-favdog` and applies `player_spread < 0` (favorite) / `player_spread > 0` (underdog) filter. Game log table Spread column now shows signed `player_spread` instead of `spread_abs`.

**Completed backtests (March 12–13, 2026):**
- **H8 — Positional DvP Validity:** ✅ RAN — REVERT verdict. Full results in `docs/BACKTESTS.md`. Prompt cleanup ✅ DONE (W6, March 13) — `DvP [POS]` line removed; `opp_today=` restored on PTS/AST stat lines.
...
- **H16 — 3PA Volume Gate:** ✅ IMPLEMENTED (`--mode 3pa-volume-gate`, March 13). Verdict pending — re-run at ~150+ 3PM picks. See `docs/ROADMAP_resolved.md`.
- **H17 — Spread Context:** ✅ IMPLEMENTED (`--mode spread-context`, output: `data/backtest_spread_context.json`, March 13). First run NOISE verdict (1.2pp on current binary split). Re-run at ≥500 picks. See `docs/ROADMAP_resolved.md`.
...
- **H16 — 3PA Volume Gate:** (`--mode 3pa-volume-gate`) Implemented March 13. First run (63 picks) showed all whitelisted players are high-volume — low/mid buckets insufficient. Re-run when 3PM pick count reaches ~150+.
- **H17 — Spread Context:** (`--mode spread-context`) First run March 13 (327 picks). NOISE — best threshold gap 4.8pp. Re-run at ≥500 picks to confirm before closing.
