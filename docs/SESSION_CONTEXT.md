...
- `morning_implied_prob` field: float — FanDuel market implied probability from the morning odds snapshot. Set once by `pretip_sweep()` before overwriting with pre-tip odds. Used by auditor for CLV calculation. `None`/absent when no pretip sweep ran for this pick's game. Added 2026-04-09.
