#!/usr/bin/env python3
"""
Backfill free-throw stats (ftm, fta) for historical rows in
player_game_log.csv.

Writes ONLY to data/player_game_log_ft_backfill.csv.
Never touches player_game_log.csv.

Run from repo root:
  python ingest/backfill_ft_stats.py
  python ingest/backfill_ft_stats.py --dry-run
  python ingest/backfill_ft_stats.py --limit 10
  python ingest/backfill_ft_stats.py --sleep 0.5
"""
...
BACKFILL_COLUMNS = ["game_id", "player_id", "ftm", "fta"]
...
def extract_ft_stats(summary_json: Dict[str, Any]) -> Dict[str, Dict[str, str]]:
    """
    Parse ESPN summary boxscore JSON → {athlete_id: {ftm, fta}}.
    ...
    for team_block in players_blocks:
        ...
        for grp in stats_groups:
            ...
            for a in athletes:
                ...
                ftm, fta = parse_made_attempted(ft_raw)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Backfill ftm/fta for historical player_game_log.csv rows. "
            "Writes to data/player_game_log_ft_backfill.csv only."
        )
    )
    parser.add_argument(
        "--dry-run",
    ...
    # ------------------------------------------------------------------
    print(f"[backfill_ft] Reading {GAME_LOG_PATH}...")
    all_rows = load_game_log()
    print(f"[backfill_ft] Total rows: {len(all_rows)}")

    filtered = filter_rows(all_rows)
    print(f"[backfill_ft] After regular-season + All-Star filter: {len(filtered)} rows")
    ...
    print(f"[backfill_ft] Unique game_ids in filtered log: {len(game_ids_all)}")
    print(f"[backfill_ft] Already backfilled (will skip):  {len(already_done)}")
    print(f"[backfill_ft] Remaining to fetch:              {len(game_ids_todo)}")
    ...
    if args.limit is not None:
        ...
        print(f"[backfill_ft] --limit {args.limit} applied; fetching {len(game_ids_todo)} game_ids")

    if not game_ids_todo:
        print("[backfill_ft] Nothing to fetch. Exiting.")
        return
    ...
    try:
        for i, gid in enumerate(game_ids_todo, start=1):
            ...
            # Fetch ESPN summary
            ft_map: Dict[str, Dict[str, str]] = {}
            fetch_ok = False
            try:
                summary  = fetch_summary(session, gid)
                ft_map   = extract_ft_stats(summary)
                fetch_ok = True
            except Exception as exc:
                print(f"[backfill_ft] WARNING: fetch failed for game_id={gid}: {exc}")
            ...
            for pid, is_dnp in players_for_game.items():
                if is_dnp or not fetch_ok:
                    # DNP rows and fetch-failure rows → empty FT stats
                    ...
                    stats = ft_map.get(norm_game_id(pid), {})
                    row_out = {
                        "game_id":   gid,
                        "player_id": pid,
                        "ftm": stats.get("ftm", ""),
                    ...
                    if not any(row_out[c] for c in ["ftm", "fta"]):
                        total_empty_rows += 1
            ...
            if i % 25 == 0:
                print(f"[backfill_ft] {i}/{len(game_ids_todo)} games fetched...")
