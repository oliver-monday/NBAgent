...
- **Home/away inversion bug — structural fix shipped (2026-04-25):** Fix 1 (manual season-context correction in DEN-MIN G3 paragraph), Fix 2 (`agents/playoff_matchup.py` TONIGHT line + `load_today_host()` helper + `today_host` series field), Fix 3 (per-player `today_is_home` field in `quant.py` + ` [H]`/` [A]` tag in `build_quant_context()` player headers + explanation paragraph in both `build_prompt()` and `build_pick_prompt()` QUANT STATS sections), and Fix 4 (`reconcile_game_attribution()` post-processor in `save_picks()` chain) all landed. Three independent stages now catch the same class of error; any single layer prevents recurrence and all three together make recurrence essentially impossible. **Investigation of `agents/season_context_updater.py`** pending as next Code prompt — determine whether the 2026-04-23 DEN-MIN G3 paragraph's two factually wrong forward-venue claims were auto-generated, and if so harden the writer against forward-venue hallucination (e.g. constrain forward statements to facts derivable from `nba_master.csv` + 2-2-1-1-1 format). Full diagnosis: `data/home_away_root_cause_investigation.md`.
