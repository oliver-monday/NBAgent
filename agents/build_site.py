...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    /* When two toggle wraps are stacked (HC + Kalshi as of P4), give the
       second one an 8px gap. Adjacent-sibling selector means the rule has
       zero effect on a single isolated wrap. */
    .high-conv-toggle-wrap + .high-conv-toggle-wrap {{ margin-top: 8px; }}
...
<div id="tab-picks" class="page active">
  ...
  <div class="high-conv-toggle-wrap">
    <label class="toggle-switch">
      <input type="checkbox" id="kalshi-toggle" />
      <span class="slider"></span>
    </label>
    <span class="hc-toggle-label">Kalshi-listed only</span>
    <span class="hc-toggle-counter" id="kalshi-counter"></span>
  </div>
...
function renderPicks() {{
  ...
  games.forEach((g, gi) => {{
    ...
    g.picks.sort((a,b) =>
      ...
      const kalshiAttr = (p.kalshi_market_listed === true) ? 'true' : 'false';
      html += `
        <div class="pick-card${{voidedCls}}" data-hc="${{hcAttr}}" data-kalshi="${{kalshiAttr}}" data-canonical="true">
          <div class="pick-main">
...
function renderTopPicks() {{
  ...
  tops.forEach(p => {{
    ...
    const kalshiAttr = (p.kalshi_market_listed === true) ? 'true' : 'false';
    html += `
      <div class="top-pick-card pick-card" data-hc="${{hcAttr}}" data-kalshi="${{kalshiAttr}}" style="border-left-color:${{color}}">
        <div class="tp-left">
...
function renderBestBets() {{
  ...
  bets.forEach(p => {{
    ...
    html += `
      <div class="best-bet-card pick-card" data-hc="${{hcAttr}}" data-kalshi="${{kalshiAttr}}" style="border-left-color:${{borderColor}}">
        <div class="tp-left">
          <div class="tp-player">${{p.player_name}}${{hcBadge}}${{buildClvWarnBadge(p)}}</div>
          <div class="tp-meta">${{p.team}}${{gameTime}}</div>
          ${{reasoning}}
...
// ── UNIFIED FILTER TOGGLES (HC + Kalshi as of P4) ──
// In-memory only — toggles reset to OFF on page reload (no localStorage).
// Two independent toggles (HC: market_implied_prob ≥ 85.0; Kalshi: pick has
// kalshi_market_listed === true) compose with logical AND: a card is shown
// iff it passes EVERY active filter. When neither toggle is on, all cards
// show. Game-group / Top Picks / Best Bets containers hide when their
// filtered pick set is empty AND any filter is active. Both counters show
// the same "Showing X of Y picks" message whenever any filter is active.
// Counter denominator uses [data-canonical="true"] cards only so Top Picks
// and Best Bets duplicates don't inflate the count.
(function() {{
  const hcToggle      = document.getElementById('hc-toggle');
  const hcCounter     = document.getElementById('hc-counter');
  const kalshiToggle  = document.getElementById('kalshi-toggle');
  const kalshiCounter = document.getElementById('kalshi-counter');
  if (!hcToggle && !kalshiToggle) return;

  function applyAllFilters() {{
    const hcOn     = hcToggle     ? hcToggle.checked     : false;
    const kalshiOn = kalshiToggle ? kalshiToggle.checked : false;
    const anyOn    = hcOn || kalshiOn;

    // Visibility loop — walks ALL pick-cards (including Top Picks / Best Bets
    // duplicates) so duplicate cards hide correctly when any toggle is ON.
    const cards = document.querySelectorAll('.pick-card');
    cards.forEach(card => {{
      // Skip opportunity cards — they don't carry data-hc / data-kalshi
      if (!card.hasAttribute('data-hc')) return;
      const passHC     = !hcOn     || card.dataset.hc     === 'true';
      const passKalshi = !kalshiOn || card.dataset.kalshi === 'true';
      const show = passHC && passKalshi;
    ...
    // Hide Top Picks header + grid + divider if no top picks pass any active filter
    ...
    if (topGrid) {{
      const visTops = Array.from(topGrid.querySelectorAll('.pick-card[data-hc]'))
        .filter(c => c.style.display !== 'none');
      const hide = anyOn && visTops.length === 0;
    ...
    if (bbGrid) {{
      const visBets = Array.from(bbGrid.querySelectorAll('.pick-card[data-hc]'))
        .filter(c => c.style.display !== 'none');
      const hide = anyOn && visBets.length === 0;
      [bbGrid, bbHeader, bbDivider].forEach(el => {{
        if (el) el.style.display = hide ? 'none' : '';
      }});
    }}

    // Counter: same "Showing X of Y picks" message on each counter when
    // any filter is active. Both blank when nothing is active.
    const message = anyOn ? `Showing ${{shown}} of ${{total}} picks` : '';
    if (hcCounter)     hcCounter.textContent     = message;
    if (kalshiCounter) kalshiCounter.textContent = message;
  }}

  if (hcToggle)     hcToggle.addEventListener('change', applyAllFilters);
  if (kalshiToggle) kalshiToggle.addEventListener('change', applyAllFilters);
  applyAllFilters();
}})();
