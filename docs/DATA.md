...
`skip_reason` values (for both `skipped_picks.json` and `skipped_picks_archive.json`):
- **LLM-emitted** (analyst Claude call writes these directly): `no_market` / `min_floor_tier_step` / `volatile_weak_combo` / `blowout_secondary_scorer` / `3pm_trend_down_tough_dvp` / `3pm_trend_down_low_minutes` / `3pm_blowout_trend_down` / `ast_hard_gate` / `fg_margin_thin_tier_step` / `reb_floor_skip` / `merit_below_floor` / `fg_cold_tier_step` / `volatile_ast_t6` / `blowout_t25_skip`
- **Python-emitted (post-processor)**: `market_gate_reject` (added 2026-05-01) — written by `enforce_market_gate()` in `agents/analyst.py` when the LLM violates the MARKET AVAILABILITY GATE and emits a pick for a player/prop/tier with no FanDuel market. The new value is NOT in the LLM-visible `skip_reason` enum in `build_prompt()` / `build_pick_prompt()` — when the LLM correctly detects no market, it emits `no_market` itself; `market_gate_reject` only fires when Python catches a violation. `rule_context.failure_mode` is one of `{player_absent, tier_absent}`. For `player_absent` (player not in FanDuel `odds_available.json` data at all), `rule_context.available_tiers = null`. For `tier_absent` (player exists but pick's tier not in the prop's tier ladder), `rule_context.available_tiers` is a sorted list of integer tiers that DID exist for that player+prop. Both auditor-grading and `skipped_picks_archive.json` schema are unchanged — the new value flows through identically to LLM-emitted skip reasons.
...
### miss_classification_report.md
Manually generated by `tools/miss_classification.py` (read-only descriptive analysis over `data/audit_log.json`). Markdown only — no schema. Idempotent: re-running produces byte-identical output (audit-log date range is the version anchor; no clock-time stamps in body). 9-section planning document covering 7-class taxonomy breakdown, 4-bucket roadmap roll-up, keyword pattern frequency within each classification, prop-type / player concentration, miss margin distribution, regular-season vs playoff split, and a non-directive findings + workstream-recommendation section. Decision input for the next major workstream choice; report explicitly does not prescribe rule changes (any rule candidate named requires its own backtest before shipping). Companion to `data/clv_analysis_YYYY-MM-DD.md` from `tools/clv_analysis.py` — both are manual-run research artifacts, not consumed by any production agent.

### injury_profiles.json
Written daily by `agents/injury_profiles.py` in `analyst.yml`. Per-player availability metrics + current injury status. Data provider only — no automated risk classification.
```json
{
  "generated_at": "ISO timestamp",
  "game_log_through": "YYYY-MM-DD",
  "total_players": 68,
  "status_summary": {"OUT": 0, "DOUBTFUL": 0, "QUESTIONABLE": 0, "ACTIVE": 68},
  "players": {
    "Player Name": {
      "team": "abbrev",
      "current_status": "OUT|DOUBTFUL|QUESTIONABLE|ACTIVE",
...
```
`current_status` derived from `injuries_today.json` overlay — no automated risk classification. Curated risk assessment lives in `### PLAYOFF INJURY LANDSCAPE` section of `context/nba_season_context.md`.
