...
- **`agents/build_site.py` — Research tab (Player Explorer)** — fifth tab added to the static SPA. `build_explorer_data()` reads `player_game_log.csv` + `nba_master.csv`; filters to active whitelist players; excludes DNPs; joins on `(game_date_str, team_abbrev)` for spread_abs and game result; computes rest_bucket (0=B2B, 1, 2, 3+); embeds per-player game arrays as `DATA.explorer` JSON in the HTML. `renderResearch()` builds 7-dropdown filter UI (player, stat, H/A, rest, spread bucket, result, opponent). `runExplorer()` applies filters, renders tier hit rate table with bar charts, distribution stats, and full game log table sorted newest-first. Fully static — no LLM calls, works on GitHub Pages.
