...
- **Top Picks — iron_floor badge** — `🔒 Iron Floor` badge in top pick cards will never fire until `iron_floor` is added to the analyst output schema in `picks.json`. Field is handled gracefully (badge just never renders). Future: add `iron_floor` as an analyst output field sourced from quant's bounce-back profiles.
