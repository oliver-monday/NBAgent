...
│   ├── season_context_updater.py # Auto-appends per-game diary entries + injury status updates to nba_season_context.md after each playoff slate; provides trim_completed_series_for_llm() for LLM input cost reduction (read-only on source file)
...
| auditor.py | claude-sonnet-4-6 | 2048 | picks.json, skipped_picks.json, player_game_log, post_game_news.json, nba_season_context, standings, picks_review_YYYY-MM-DD.json (optional) | audit_log.json (per-day entry + new `high_conviction_breakdown` field with n/hits/misses/hit_rate/delta_vs_overall for picks with market_implied_prob ≥ 85.0), audit_summary.json (rolling stats + new `high_conviction_summary` top-level key), updates picks in-place, grades skipped_picks.json (parlay grading removed 2026-04-24; high-conviction breakdown added 2026-04-25) |
...
- **`context/nba_season_context.md`** is injected into BOTH `analyst.py` and `auditor.py` prompts. Updates to this file affect both agents. **During playoffs it is auto-updated daily** by `season_context_updater.py` (runs in `auditor.yml` after `auditor.py`, date-gated to `PLAYOFFS_R1_DATE = 2026-04-18`): appends new series diary entries based on yesterday's completed postseason games, patches injury bullets where game performance changes status, and bumps the timestamp. Manual edits still work — the agent only appends within existing `##### (N) TEAM vs (N) TEAM` sections and replaces specific bullet lines it's asked to update. **For LLM input, completed playoff series diaries are condensed in-memory** to one-line digests via `trim_completed_series_for_llm()` (in `season_context_updater.py`), driven by the `completed: true` flags in `data/playoff_bracket.json`. The source markdown file is unmodified — both `analyst.load_season_context()` and `auditor.load_season_context()` read full text from disk then apply trim before returning. Idempotency ensured via a `<!-- TRIMMED -->` sentinel in the digest header. Pure pass-through on any error (missing/malformed bracket, no completed series, etc.) — full untrimmed text returned with a warning.
...
| Tab | Content |
|-----|---------|
| Today's Picks | **High-conviction toggle** at top filters to picks with market_implied_prob ≥ 85.0 (in-memory only, resets on reload; counter shows "Showing X of Y picks" while ON). **HIGH CONV** green badge on every qualifying card. Injury report dropdown, pick cards grouped by game (collapsible). Voided picks show strikethrough + VOIDED badge. DOUBTFUL/QUESTIONABLE picks show risk pills. Lineup Update shows ↑/↓ badge with expandable amendment detail. Review badges: ⚠ Caution (amber, trim verdict) and ⚠ Flagged (red, manual_skip verdict) shown below status badge when picks_review file present. **Edge tier label** (renamed 2026-04-25 from FADE/NEUTRAL/POSITIVE/STRONG → MARKET LOCKED / MARKET CONFIDENT / JUDGMENT CALL / RARE SETUP) derived at render time from `calibrated_edge_pct` directly via `tierLabelFromEdge()` JS helper using new boundaries (< -10pp / -10..-5 / -5..+5 / ≥ +5). Best Bets section below Top Picks: picks with `calibrated_edge_pct >= 5.0` (RARE SETUP band) ranked by edge; teal (5–8pp) or green (≥8pp) border; includes Odds + Sizing drawer. |
...
| Audit Log | Latest auditor entry — hit rate stats, what worked, what to avoid, analyst instructions. **High-Conviction panels (added 2026-04-25):** two cards adjacent to the main hero — yesterday subset (from `last_audit.high_conviction_breakdown`) + rolling subset (from `audit_summary.high_conviction_summary`); both show n_picks, hits, misses, hit_rate, delta_vs_overall_pp; threshold market_implied_prob ≥ 85.0. Skip validation table. |
