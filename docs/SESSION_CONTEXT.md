...
**Last updated:** April 11, 2026 ((56) **Lineup update: filter OUT/DOUBTFUL players from opportunity flags** — `agents/lineup_update.py` `build_opportunity_suggestions()` now accepts an optional `injuries: dict | None = None` kwarg (default preserves backward compat). When provided, the function builds an `excluded_players` set keyed by `name_lower` from any injury row with status OUT/DOUBTFUL/OFS, and the scanning loop skips any candidate player whose name is in that set. `main()` call site at line 1386 now passes the in-scope `injuries` dict. Root-cause incident on 4/10: Jokic's absence triggered an opportunity scan on DEN/OKC, surfacing SGA, Holmgren, Hartenstein, and Jalen Williams as opportunities despite all four being OUT for rest (Quickley/TOR had the same issue from a separate absence). Unit-tested with a synthetic Jokic-OUT scenario exactly matching the 4/10 pattern: pre-fix surfaced 5 suggestions (4 of them OUT players), post-fix surfaced 1 (the legitimate DEN teammate Jamal Murray), zero OUT players in post-fix output. Existing self-exclusion guard (skip the absent player themselves) is unchanged — this is an additional filter on the candidate side. `agents/lineup_update.py` only — signature + docstring + excluded_players build + loop check + call site kwarg. Not touched: `compute_lineup_diff()`, `save_opportunity_flags()`, `opportunity_flags.json` schema, `classify_absent_player()`, `_norm()`.)
