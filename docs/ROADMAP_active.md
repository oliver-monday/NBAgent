...
- **Home/away inversion bug — partial fix (2026-04-25):** Fix 1 (manual season-context correction in `context/nba_season_context.md` DEN-MIN G3 paragraph — replaced "must-win road game" attributing road status to MIN with "must-win road game" attributing it to DEN, and "MIN will close out the series at home in G5" with "MIN will close out the series on the road in G5") shipped. **Fix 2/3/4 pending** as a combined Code prompt: (Fix 2) `agents/playoff_matchup.py` series_context emitter to add an explicit "TONIGHT: Game N at <host>" anchor under each `game_today=True` series header; (Fix 3) `agents/analyst.py:build_quant_context()` to surface a per-player `[H]`/`[A]` venue tag derived from the already-populated `playoff_series_state.is_road_game` field in `player_stats.json`; (Fix 4) `agents/analyst.py:save_picks()` to add a `reconcile_game_attribution()` post-processor that overwrites `home_away` and `opponent` from `nba_master.csv` before writing `picks.json` (defense-in-depth on the structured fields). **Investigation of `agents/season_context_updater.py`** pending as a third Code prompt — determine whether the auto-generator produced the two wrong DEN-MIN G3 sentences, and if so harden it against forward-venue hallucination (e.g. constrain forward statements to facts derivable from `nba_master.csv` + 2-2-1-1-1 format). Full diagnosis: `data/home_away_root_cause_investigation.md`.
