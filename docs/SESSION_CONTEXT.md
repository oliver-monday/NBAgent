...
**Last updated:** March 22, 2026 (H19 Finding 2 applied: `3PM blowout trend-down hard skip` (spread_abs 8–18) retired in `build_pick_prompt()`. trend=down AND BLOWOUT_RISK=True at spread_abs 8–18 now defers to the existing trend=down step-down rule only — no hard skip. The spread_abs ≥ 19 unconditional hard skip is unchanged. `build_prompt()` (fallback) is NOT updated. Backtest evidence: H19 n=96–137, lift=1.097–1.103 — blowout_win 3PM hits at 78.8–79.2%, above 71.8% baseline. `agents/analyst.py` only.)
...
- `compute_teammate_absence_splits(player_log, player_name, team, whitelist)` → `{teammate_name, teammate_avg_pts, n_games, raw_avgs, tier_hit_rates}` or `None` — computes player's raw avgs and tier hit rates in games where their top-PPG whitelisted teammate was absent (min 3 qualifying games); stored as `key_teammate_absent` field in `player_stats.json`; used by `build_quant_context()` to emit `Without [X]` line; added 2026-03-20
...
- `build_scout_prompt(games, player_context, injuries, season_context, quant_context, pre_game_news, player_profiles, playoff_picture, team_defense, leaderboard, lineups_section)` → Scout stage prompt; all contextual data, NO rules; output schema: `{"slate_read": str, "shortlist": [...], "omitted": [...]}`; targets 20–25 players; added 2026-03-20
- `call_scout(prompt, model=MODEL) → tuple[list[dict] | None, list[dict]]` — calls Claude for Scout shortlist; returns `(shortlist, omitted)` tuple on success, `(None, [])` on any failure (triggers fallback); never calls `sys.exit()`; uses `SCOUT_MAX_TOKENS = 4096`; streaming API; same JSON repair pattern as `call_analyst()`; added 2026-03-20; tuple return added 2026-03-20
- `save_scout_omitted(omitted: list[dict]) → None` — writes `data/scout_omitted_YYYY-MM-DD.json` with Scout's omitted block; called in `main()` immediately before Stage 2 Pick call; added 2026-03-20. **Omitted schema (updated 2026-03-25):** requires one entry per non-shortlisted quant stats player — every player in the QUANT STATS section must appear on shortlist OR in omitted (no exceptions). Two categories: (1) hard exclusions (players not shortlisted at all: no qualifying tier, confirmed OUT, role eliminated, or simply outcompeted by stronger candidates) and (2) deprioritized candidates (shortlisted at `priority: "medium"` with meaningful situational risk — B2B, VOLATILE sole prop, thin minutes floor, QUES designation, extreme blowout as secondary scorer); `risk_type` field (`hard_exclusion` | `deprioritized` | `none`). Sentinel entry `{"player_name": "none", ..., "risk_type": "none"}` valid ONLY when every quant stats player is on the shortlist. `call_scout()` injects sentinel and logs `WARNING` if model returns `[]` (prompt compliance failure). Previously the array was always `[]` because the prompt only said "must not be empty" without requiring per-player accounting.
...
- `volatile_weak_combo` (2026-03-23 update): **PTS props only** — do NOT apply to AST picks below T6 (governed by the VOLATILE AST skip rule at its own T6 threshold). Two exceptions now in effect: **(a)** player has [iron_floor] on this stat AND trend=up; **(b)** stat is AST AND raw_avgs AST ≥ 6.0 AND [iron_floor] is true on AST — elite passers (Jokic-tier) with a confirmed structural floor are not weak-combo candidates; apply standard −5% VOLATILE deduction and evaluate normally, emit pick not skip. Motivated by 100% false skip rate on Jokic T8 AST and Barnes T4 AST. Both `build_prompt()` and `build_pick_prompt()` updated identically.
...
**`SCOUT_OMITTED_JSON` and other date-stamped module constants (2026-03-21):** Module-level path constants that embed `TODAY_STR` (e.g. `DATA / f"scout_omitted_{TODAY_STR}.json"`) must be defined AFTER the `TODAY` / `TODAY_STR` block (lines ~46–47). Placing them above causes a `NameError` on import. All other date-stamped paths in the file are in functions or after the date block — keep this invariant.
...
## Active Improvement Queue (as of March 20, 2026)
