...
**Phase 1.5 — Prefetch + market availability gate (2026-04-07).** `--prefetch` mode added to `ingest/odds_today.py`. Fetches ALL available FanDuel alternate player prop markets for today's games (filtered to games with whitelisted players) BEFORE the analyst runs. Writes `data/odds_available.json` — consumed by `agents/analyst.py` as a market-availability gate: if no FanDuel market exists for a player+prop+tier, the analyst is forbidden from generating that pick. Eliminates picks on lines that have no corresponding betting market. Credit cost identical to Phase 1 (4 credits/game). Analyst integration COMPLETE (2026-04-07) — unconditional hard rule: no FanDuel market → no pick.

**Phase 1.75 — Calibration-Corrected Edge Enrichment (2026-04-07).** `compute_edge()` function added to `ingest/odds_today.py`. Reads confidence calibration bands from `audit_summary.json` (`confidence_calibration` section), maps each pick's stated confidence to the band's actual historical hit rate (calibrated probability), computes edge vs market implied probability, derives quarter-Kelly fraction, and assigns a recommendation tier. Writes `bet_recommendation` sub-object to each today's pick in `picks.json`. Tiers: STRONG (>+8pp), POSITIVE (+3–8pp), NEUTRAL (±3pp), FADE (<−3pp), NO_MARKET (no odds data). Graceful degradation: if `audit_summary.json` missing or has no calibration data, falls back to stated confidence as probability. Runs as part of the existing `main()` flow in `analyst.yml` — no workflow changes needed. `ingest/odds_today.py` only.

**Phase 1.8 — Parlay Agent Edge Awareness (2026-04-07).** `build_parlay_prompt()` in `agents/parlay.py` extended to surface `bet_recommendation` data on each leg. Two new fields in slim leg construction: `edge_tier` (`recommendation_tier` from `bet_recommendation` sub-object) and `cal_edge` (`calibrated_edge_pct`). Criterion #6 (Edge quality) added to SELECTION CRITERIA: prefer STRONG/POSITIVE edge legs, avoid FADE legs, max 1 FADE per parlay with all other legs POSITIVE/STRONG to compensate. AVOID section extended with compound FADE prohibition. Advisory only — no Python hard gate; Claude uses edge data as a selection signal alongside existing confidence/correlation criteria. Graceful degradation: `edge_tier` and `cal_edge` are `null` when `bet_recommendation` is absent (pre-enrichment picks or NO_MARKET). `agents/parlay.py` only.

**Phase 1.9 — Frontend Odds Integration Display (2026-04-07).** `agents/build_site.py` extended with odds/edge/sizing UI on pick cards. Two new elements per pick: (1) colored recommendation tier label with calibrated edge (STRONG green / POSITIVE teal / NEUTRAL muted / FADE red) in the right column below confidence line; (2) expandable "Odds + Sizing" drawer in left column showing market line, market prob, calibration band, calibrated prob, edge (pp), and quarter-Kelly fraction. Top Picks cards also show the tier label. All rendering gracefully handles missing `bet_recommendation` — shows nothing when absent or NO_MARKET. New CSS: `.edge-line`, `.odds-sizing-toggle`, `.odds-sizing-body`. New JS: `toggleOddsSizing()`, `buildEdgeLine()`, `buildOddsSizing()`. `agents/build_site.py` only.

**Phase 2 — Decision support UI (offseason / next season):** "Market disagrees" flag when market implied prob is >10pp above system confidence. Per-pick actionable summary number for users without statistics background. Extends Phase 1.9 display with contextual warnings.
